...
def generate_default_anchor_maps(anchors_setting=None, input_shape=cfg.INPUT_SIZE):
    """
    ...
    for anchor_info in anchors_setting:
        ...
        for scale in scales:
            for aspect_ratio in aspect_ratios:
                ...
                center_anchors = np.concatenate((center_anchors, center_anchor_map.reshape(-1, 4))) #cxcywh
                edge_anchors = np.concatenate((edge_anchors, edge_anchor_map.reshape(-1, 4))) #xyxy
                anchor_areas = np.concatenate((anchor_areas, anchor_area_map.reshape(-1))) #area
